...
class UserReport extends Component
{
    ...
    public function fetchLogs()
    {
        ...
        $this->logsGrouped = $logs->groupBy(function($log) {
            return $log->creator->name ?? 'Unknown'; // group by user name
        })->map(function($userLogs, $userName) {
            return [
                'user' => $userName,
                'entries' => $userLogs->map(function($log) {
                    return [
                        'log_date' => $log->log_date,
                        'activity' => $log->activity,
                        'duration_number' => $log->duration_number,
                        'duration_unit' => $log->duration_unit,
                    ];
                })->toArray(),
            ];
        })->values()->toArray();
